 ...
 */
uint8_t imu_read_register(uint8_t reg_addr, uint8_t* data, uint16_t data_len) {
 ...
 * @return 1 on success, 0 on I2C communication error.
 */
uint8_t imu_write_register(uint8_t reg_addr, uint8_t reg_data) {
 ...
 */
float get_accel_sensitivity(uint8_t accel_fs) {
    switch(accel_fs) {
 ...
 */
float get_gyro_sensitivity(uint8_t gyro_fs) {
    switch(gyro_fs) {
