                   ...
                   uint32_t timeout_ms)
{
	 ...
	 */
	HAL_StatusTypeDef spi_error_hal = 0;
...
void cs_select(void){
	...
	HAL_GPIO_WritePin(SPI2_CS_NAND_GPIO_Port, SPI2_CS_NAND_Pin, 0);
}

void cs_deselect(void){
	/** \brief Put chip select high to disable SPI communication with NAND */
	HAL_GPIO_WritePin(SPI2_CS_NAND_GPIO_Port, SPI2_CS_NAND_Pin, 1);
}
...
{
	HAL_StatusTypeDef spi_error_hal = 0;
	spi_error_hal = HAL_SPI_Receive(&hspi2, read_buff, read_len, timeout_ms);
    return SPI_NAND_RET_OK;
}
