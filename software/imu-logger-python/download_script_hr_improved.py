...
def process_bin_file(bin_filename, csv_filename=None):
    ...
    # Estimate heart rate from the detected beat markers.
    # HR is computed as beats per minute:
    # HR = number_of_detected_beats / analyzed_duration_seconds * 60
    if len(ir_time) > 1:
        analyzed_duration_s = float(ir_time.iloc[-1] - ir_time.iloc[0])
    else:
        analyzed_duration_s = float(plot_duration_s)

    if analyzed_duration_s <= 0:
        analyzed_duration_s = float(plot_duration_s)

    num_beats = len(beat_indices)
    estimated_hr = (num_beats / analyzed_duration_s) * 60.0

    # The minimum distance also defines the maximum detectable HR:
    # HRmax = 60 / min_distance_s
    hr_max = 60.0 / min_distance_s

    print(f"Detected beats: {num_beats}")
    print(f"Analyzed duration: {analyzed_duration_s:.2f} s")
    print(f"Estimated HR: {estimated_hr:.1f} bpm")
    print(f"Maximum detectable HR with min_distance_s={min_distance_s:.2f} s: {hr_max:.1f} bpm")
    ...
    plt.title(f"IR raw PPG and estimated beats - HR ≈ {estimated_hr:.1f} bpm")
    plt.xlabel("Time [s]")
